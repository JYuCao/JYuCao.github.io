...
> $  \left\{
    \begin{aligned}
      \int_0^{2\pi} \cos^n xdx=4\int_0^{\frac{\pi}{2}}\sin^n xdx=4\int_0^{\frac{\pi}{2}}\sin^n xdx(n\text{ 为偶数}) \\
      \int_0^{2\pi} \cos^n xdx=\int_0^{2\pi}\sin^n xdx=0(n\text{ 为奇数})
    \end{aligned}
  \right.$<br><br>
...
> 2. 高辅P124例3（瑕积分判别）<br>
> $\lim_{x\rightarrow0}x^\xi \ln x = 0(\xi\text{ 为任意正常数})$<br>
